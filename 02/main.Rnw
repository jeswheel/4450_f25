...
\subsection{Binomail Distribution}
...
\begin{frame}[allowframebreaks]{Binomial Distribution}
  ...
  Suppose that we have $n$ independent experiments, where $n$ is a fixed (positive) integer.
  ...
  \emph{Question:} What is the probability that $X = k$, for some $k \in \{0, 1, 2, \ldots\}$?
  ...
  \mode<article>{
    ...
    For $X = k$, we must have \emph{exactly} $k$ successes and $n-k$ failures. By the multiplication law, any one such sequence has probability $p^k(1 - p)^{n-k}$ (for instance suppose $n = 3$. What is the probability of the event $SFS$? it's $p \times (1-p) \times p = p^2(1-p)^{3-2}$.)
    
    Because we only care about the \emph{number} of successes, not the order, we now have a counting problem: with $n$ total trials (positions), how many ways can we arrange the $k$ successes? Another way of thinking is: ``How many ways can we choose $k$ out of $n$ locations in a sequence to place the successes?".
  ...
  \begin{exampleblock}{Rolling Dice}
    ...
    Let $X$ denote the number of 6s in this experiment, which takes values in the set $\{0, 1, \ldots 5\}$.
  ...
  \emph{Note:} A binomial RV can be expressed as the sum of independent Bernoulli RVs. That is, let $X_1, X_2, \ldots, X_n$ be independent Bernoulli RVs, each with $P(X_i = 1) = p$. Then,
...
\begin{frame}[allowframebreaks]{Geometric Distribution}
  ...
  We can construct a \alert{geometric} RV in a similar way that we did with the binomial distribution.
...
\begin{frame}[allowframebreaks]{Negative Binomial Distribution}
  The \alert{negative binomial} (NB) distribution can be thought of as a generalization of the geometric distribution; rather than stopping when we have exactly one success, we now will stop when we have $r$ successes.
  
  For any particular sequence of trials of length $k$ that satisfy this condition, the probability is $p^r(1 - p)^{k - r}$.
  Now, the last trial must be a success (because we stopped), so we need to choose the location of the remaining $r - 1$ successes. Thus, if $X$ has a negative binomial distribution, the pmf is:
  $$
...
\subsection{Hypergeometric and Poisson Distribution}
...
\begin{frame}[allowframebreaks]{The Poisson Distribution}
  ...
  The value of $\lambda$ controls the \emph{shape} of the distribution:
...
\begin{frame}{Density function}
  ...
  Instead, the role of the pmf is taken by a \alert{probability density function} (pdf), which we will denote $f(x)$.
...
\begin{frame}{Probabilities}
  ...
  $$
  An immediate consequence of this definition is that $P(X = a) = 0$ for any $a \in \R$.
  
\end{frame}

\begin{frame}[allowframebreaks]{Example: Continuous uniform random variable}
  By \emph{uniform}, we mean that all outcomes in the given set are equally as likely.
...
\begin{frame}[allowframebreaks]{Percentiles}
  You're probably familiar with the term \alert{median}.
  ...
  Formally, the sample median is the same as the definition above, using the \emph{empirical} distribution function \citep{wiki:edf}.
...
\begin{frame}[allowframebreaks]{Exponential Distribution}
  ...
  When this is the case, we refer to it as the \alert{family} of exponential densities that is \emph{indexed} by the parameter $\lambda$. 
