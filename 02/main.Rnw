...
\begin{frame}[allowframebreaks]{Identically distributed}
  \begin{itemize}
    \item Let $X$ and $Y$ be random variables defined on the same space.
    \item We say that $X$ and $Y$ are \alert{identically distributed} if for all (measureable) subsets $A \subset \R$, $P(X \in A) = P(Y \in A)$.
  \end{itemize}
  \begin{block}{Theorem~\CHAPTER.1: Identical distributions}
  ...
  \begin{itemize}
    ...
    $$
    \item From Theorem~\CHAPTER.1, we have the equivalency between cdf's and distributions. Naturally, we want to extend this to pdf's as well, but there is a subtle distinction that must be made.
