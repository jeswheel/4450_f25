...
\end{itemize}
  ...
  \begin{block}{Theorem~3.1: Order statistic for continuous random variables}
  ...
  \begin{block}{Theorem~3.2: The pdf of ordered statistics}
    Consider the same setup as Theorem~3.1. That is, $X_{(1)}, \ldots, X_{(n)}$ represent the order statistics of a random sample. Then the pdf of $X_{(j)}$ is
    $$
  ...
  \begin{block}{Theorem~3.3: Discrete random variables}
  ...
  \begin{block}{Theorem~3.4: Joint distributions}
