...
\begin{exampleblock}{Example: Coupon Collection}
  Suppose that you collect coupons, that there are $n$ distinct coupons, and that on each trial you are equally likely to get a coupon of any of the types.
  TODO: finish example. 
\end{exampleblock}
