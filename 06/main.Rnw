...
\begin{figure}[ht]
<<tDistFig, echo=FALSE, fig.height=2.5, fig.width=5>>=
df <- data.frame(
  x = seq(-3.2, 3.2, length.out = 1000)
)

df$val_Normal <- dnorm(df$x)
df$val_t_2 <- dt(df$x, 2)
df$val_t_5 <- dt(df$x, 5)
df$val_t_10 <- dt(df$x, 10)
df$val_t_30 <- dt(df$x, 30)
...
pivot_longer(
  ...
  mutate(Distribution = factor(Distribution, levels = c("t_2", "t_5", "t_10", "t_30", "Normal"))) %>% 
  ggplot(aes(x = x, y = value, group = Distribution, col = Distribution)) + 
  geom_line() + 
  theme_bw() + 
  scale_color_manual(
    values = c(
      "Normal" = 'black', 
      't_2' = "#c7e9c0", "t_5" = "#a1d99b", "t_10" = '#41ab5d' , "t_30" = '#006d2c' 
    )
  )
@
\end{figure}
